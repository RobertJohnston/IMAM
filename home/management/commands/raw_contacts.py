        ...
        with transaction.atomic():
            ...
            for json_contact_row in data_to_process.iterator():
                ...
                if RawRegistration.objects.filter(uuid=uuid):
                    print("Raw Contacts countdown %s UPDATING a contact for %s with uuid %s" % (counter, json_data['name'], uuid))
                    raw_contacts = RawRegistration.objects.get(uuid=uuid)
                # Create contact
                else:
                    print("Raw Contacts countdown %s    CREATING a contact for %s with uuid %s" % (counter, json_data['name'], uuid))
